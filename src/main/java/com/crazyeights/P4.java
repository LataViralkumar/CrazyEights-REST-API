...
@RestController
@RequestMapping("crazyeight/api/v1.0/Player4")
public class P4 {
	...
	private Players p4 = new Players(); 
	
	@GetMapping("/cardDistribute")  // For player one

	public LinkedList<pairStackInit> showCardPlayer4() {
		...
		p4.setPlayerCard(cartForPlayer1);
	
		return p4.getPlayerCard();  
	...
	public pairStackInit gameStarter(@RequestBody pairStackInit pair_p1){
		...
		System.out.println(this.p4.getPlayerCard());
		
		for(int i = 0; i < this.p4.getPlayerCard().size();++i) {
			
			if(p4.getPlayerCard().get(i).getCardType().equals(pair_p1.getCardType())) {
//				System.out.println("Yes..........");
				p4.RemoveCardFromPlayer(i);
				break;
			}
		}
	...
	public pairStackInit PlayerWantGetCardFromRemaingDeack() {
		...
		pairStackInit PairStackInit =  this.game.getRemaingCard().getFirst();
		p4.insertNewcard(PairStackInit);
